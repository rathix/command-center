...
<div class="mx-auto lg:max-w-[1200px]">
	<div class="flex flex-wrap items-center justify-between gap-2" role="status" aria-live="polite">
		<div class="flex items-center gap-2">
			{#if isInitialLoad}
				<span class="text-xs font-semibold text-subtext-0 sm:text-sm">Discovering services...</span>
			{:else if getCounts().total === 0 && getConnectionStatus() === 'connected'}
				<span class="text-xs font-semibold text-subtext-0 sm:text-sm">No services discovered</span>
			{:else if !getHasProblems()}
				<span class="text-xs font-semibold text-health-ok sm:text-sm"
		...
		<div class="flex items-center gap-3">
			{#if getConnectionStatus() === 'reconnecting'}
				<span class="text-xs font-semibold text-subtext-0 italic sm:text-sm">Reconnecting...</span>
			{:else if getConnectionStatus() === 'disconnected'}
				<span class="text-xs font-semibold text-health-error sm:text-sm">Connection lost</span>
			{:else if getAppVersion()}
				<span class="hidden text-xs text-subtext-0 sm:inline">Command Center {getAppVersion()}</span>
			{/if}
